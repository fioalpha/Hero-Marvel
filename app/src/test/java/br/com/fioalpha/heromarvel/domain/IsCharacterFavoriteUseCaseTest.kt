...
class IsCharacterFavoriteUseCaseTest {
    ...
    private val characters = Gson().fromJson<List<CharacterDomain>>(MockCreator().getMockText(CHARACTER_JSON))
    ...
    fun `when called execute With all item not matcher`() {
        whenever(repository.fetchCharacterFavorite()).thenReturn(
            Observable.just(
                listOf(
                    CharacterDomain(3, "Three", "Three", "Three"),
                    CharacterDomain(4, "Four", "Four", "Four")
                )
            )
        )
        isCharactersUseCase.setCharacters(characters)
